...
_MOD_PAIRS_CACHE: list[tuple[str, str]] | None = None


def scrape_mod_pairs() -> list[tuple[str, str]]:
    """Pair EN/JP mod lines position-by-position across every MOD_SOURCES page.
    Each line retains its %1..%N placeholders (from _normalize_mod). Globally
    deduped. Cached so callers (scrape_mod_patterns + scrape_mod_templates)
    don't re-parse the same HTML twice."""
    global _MOD_PAIRS_CACHE
    if _MOD_PAIRS_CACHE is not None:
        return _MOD_PAIRS_CACHE
    ...
    for path in MOD_SOURCES:
        ...
        for en_lines, jp_lines in zip(en_spans, jp_spans):
            ...
            for en, ja in zip(en_lines, jp_lines):
                ...
                seen.add((en, ja))
                out.append((en, ja))
    _MOD_PAIRS_CACHE = out
    return out


def scrape_mod_patterns() -> list[dict]:
    """Lua-pattern form for Mods.lua. EN side gets `(%d+)` captures and `^…$`
    anchors; JA side keeps `%N` backrefs for Lua gsub."""
    out: list[dict] = []
    for en, ja in scrape_mod_pairs():
...
def scrape_mod_templates() -> dict[str, str]:
    """Brace-template form for ModTemplates.lua. `%N` → `{N-1}` on both sides;
    the EN form becomes the dict key, the JA form the value. Hit by the
    Locale.TokenizeNumbers fallback inside JaText.translate at render time."""
    out: dict[str, str] = {}
    for en, ja in scrape_mod_pairs():
        en_tpl, ja_tpl = en, ja
        for n in range(9, 0, -1):  # high-to-low to avoid %1 chewing %10
            en_tpl = en_tpl.replace(f'%{n}', f'{{{n - 1}}}')
            ja_tpl = ja_tpl.replace(f'%{n}', f'{{{n - 1}}}')
        out[en_tpl] = ja_tpl
    return out
...
def write_mod_templates_lua(out_path: Path, templates: dict[str, str]) -> None:
    """Key-value dict in {N}-placeholder form, merged into T_MAP by Locale.lua
    and hit via the tokenizer fallback in JaText.translate. Parallel companion
    to Mods.lua — same data, alternate addressing scheme."""
    lines = [
        "-- Mod templates ({N}-placeholder form) from poe2db.tw/jp/Modifiers.",
        "-- Auto-generated by tools/scrape_poe2db.py.",
        "-- Companion to Mods.lua: same EN→JA pairings, but as a key-value dict",
        "-- with {0},{1},... placeholders so Locale.TokenizeNumbers can probe",
        "-- runtime strings like \"+15 to Strength\" → \"{0} to Strength\".",
        "return {",
    ]
    for en in sorted(templates):
        ja = templates[en]
...
def main(argv: list[str]) -> int:
    ...
    if not args.only or args.only == "Mods":
        ...
        try:
            ...
            templates = scrape_mod_templates()
        except requests.RequestException as e:
            print(f"  ! failed: {e}", file=sys.stderr)
            patterns, templates = [], {}
        mods_path = out_dir / "Mods.lua"
        tpl_path = out_dir / "ModTemplates.lua"
        if not args.dry_run:
            write_mods_lua(mods_path, patterns)
            write_mod_templates_lua(tpl_path, templates)
        print(f"  -> Mods.lua: {len(patterns)} patterns"
              + (" (dry-run, not written)" if args.dry_run else ""))
        print(f"  -> ModTemplates.lua: {len(templates)} templates"
              + (" (dry-run, not written)" if args.dry_run else ""))
