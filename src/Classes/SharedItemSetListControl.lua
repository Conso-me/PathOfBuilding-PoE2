...
local SharedItemSetListClass = newClass("SharedItemSetListControl", "ListControl", function(self, anchor, rect, itemsTab)
	...
	self.controls.delete = new("ButtonControl", {"BOTTOMLEFT",self,"TOP"}, {2, -4, 60, 18}, T("Delete"), function()
...
function SharedItemSetListClass:AddValueTooltip(tooltip, index, sharedItemSet)
	...
	for _, slot in ipairs(self.itemsTab.orderedSlots) do
		if not slot.nodeId then
			...
			if item then
				tooltip:AddLine(16, s_format("^7%s: %s%s", self.itemsTab.slots[slotName].label, colorCodes[item.rarity], ItemT(item.name)))
			end
		end
	end
end
...
function SharedItemSetListClass:OnSelDelete(index, sharedItemSet)
	main:OpenConfirmPopup(T("Delete Item Set"), "Are you sure you want to delete '"..(sharedItemSet.title or "Default").."' from the shared item set list?", "Delete", function()
